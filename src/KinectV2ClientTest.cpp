...
private:
  ...
  image_transport::Publisher color_joints_pub;
...
public:
    ...
    :it(nh), pnh("~")//, info_mgr( nh, "camera" )
  {
    ...
    string color_topic, color_joints_topic, depth_topic, bodyindex_topic;//, info_topic;
    color_topic = "/" + c_frame + "/image/color";
    color_joints_topic = "/" + c_frame + "/image/color_joints";
    ...
    color_joints_pub = it.advertise( color_joints_topic, 1 );
  ...
  void test()
  {
	...
	cv::Mat imageColorJoints;
	imageColorJoints = imageColor;
	KinectPackUtil::drawSkeleton( imageColorJoints, kinectPack, 
				      cols_scale, rows_scale );
	
	cv::waitKey(1);
	cv_bridge::CvImage cv_img_color,cv_img_color_joints, cv_img_depth, cv_img_bodyindex;
	...
	cv_img_color_joints.header.stamp = time;
	cv_img_color_joints.header.frame_id = c_frame;
	cv_img_color_joints.encoding = "bgr8";
	cv_img_color_joints.image = imageColorJoints;
	...
	color_joints_pub.publish( cv_img_color_joints.toImageMsg() );
	...
	name <<"/home/yhirai/okaodata/kinect_data"; 
